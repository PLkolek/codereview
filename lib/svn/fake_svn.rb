...
"Index: java/main/org/apache/jute/Utils.java
===================================================================
--- java/main/org/apache/jute/Utils.java	(wersja 687110)
+++ java/main/org/apache/jute/Utils.java	(wersja 687111)
@@ -124,8 +124,10 @@
      * @param s
      * @return
      */
...
"
  ...
  def self.cat(url, file, revision_number)
    if file=="java/main/org/apache/jute/Utils.java"
"/**
...
public class Utils {
 ...
 (Flavio Paiva Junqueira via mahadev)"
  ...
  def self.log(url, user, title)
    "r681910 | mahadev | 2008-08-02 02:07:30 +0200 (sob) | 1 linia
...
------------------------------------------------------------------------
r680431 | mahadev | 2008-07-28 20:19:53 +0200 (pon) | 1 linia

ZOOKEEPER-105. Catch Zookeeper exceptions and print on the stderr. (Anthony Urso via Mahadev)
------------------------------------------------------------------------
r674665 | mahadev | 2008-07-08 01:02:21 +0200 (wto) | 1 linia

ZOOKEEPER-68. Fix command line parsing for zookeeper barebone java client for acls. (Jakob Homan via mahadev)
------------------------------------------------------------------------
r671678 | mahadev | 2008-06-26 01:06:01 +0200 (czw) | 1 linia

ZOOKEEPER-53 : fix tests to run on solaris. (mahadev)
------------------------------------------------------------------------
"
  end
end
